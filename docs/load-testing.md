...
### 5.2 本番で回すもの(合計約 8,000 リクエスト)

| # | 形 | 件数 | 目的 |
|---|---|---|---|
| **P1** | `ramping-arrival-rate` **5 → 50 rps を 60 秒でランプ、60 秒維持**、qrId は 5 プロジェクトに Zipf 分散、`redirects: 0` | 約 3,000 | 実 D1 レイテンシでの p95/p99 分布と、リクエスト単位の CPU 時間。**想定実ピークの再現であって攻撃の模擬ではない**(§2.1) |
...
書き込み消費 ≈ 8,100 / 100,000 = **8%**。修正後の再実行が何回分も残る。
